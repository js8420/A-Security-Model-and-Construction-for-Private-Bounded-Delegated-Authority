...
/// A satisfying trace for the whole circuit at full parameters: policy
/// columns, the three C9 sponge permutations, two allowlist paths, the
/// non-revocation path, and the glue columns the cross-component bindings
/// compare against.
///
/// Coherence is what makes this worth doing. The sponge absorbs the very
/// policy values the clauses read, the allowlist roots in those columns are
/// the roots the paths actually compute, and the leaf lanes carry the merchant
/// and category the payment names. Nothing is patched to satisfy a constraint.
